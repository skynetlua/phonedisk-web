...
local function excute_cmd(cmd)
    do return end
    -- local ok, file = pcall(io.popen, cmd)
    -- if not ok then
    --     return
    -- end
...
local function start_system_record()
    local info = get_system_info()
    if not info then
        skynet.error("sorry,system no support the cmd:top -bn 2 -i -c -d 0.1")
        return
    end
    do_system_record()
end
...
function command.kick(ip)
    -- local cmd = [[grep ]] .. ip .. [[ /etc/hosts.deny > /dev/null;if [ $? -gt 0 ];then echo "sshd:]] .. ip .. [[:deny" >> /etc/hosts.deny; fi]]
    -- os.execute(cmd)
    local serverds = get_serverds()
    for _,serverd in ipairs(serverds) do
        skynet.send(serverd, "lua", "kick", ip)
    end
end
...
function command.service_stat()
    ...
    if type(meminfo) ~= "table" or not next(meminfo) then
    ...
    for _,serverd in ipairs(serverds) do
        ...
        for minute,record in pairs(server_records) do
            ...
            if not tt_record then
                ...
                tt_record.ip_count = tt_record.ip_count+record.ip_count
                tt_record.visit_times = tt_record.visit_times+record.visit_times
            end
        end
    end
    local records = {}
    for _,record in pairs(total_records) do
...
server(command, start_system_record)
